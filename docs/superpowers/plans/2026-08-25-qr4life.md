# QR4Life Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Dynamické QR kódy — tištěný kód obsahuje jen `https://qr4life.cz/{hash}`, cíl se mění v administraci.
...
- [ ] Placeholder homepage (bude nahrazen Task 11): `<main>QR4Life</main>`.
...
- Download route: pouze owner (`session.user.id === qr.userId`, jinak 404 — netesit existenci), `?format=png|svg&size=512`; obsah podle typu: url→url, wifi→wifiString, vcard→vcardString, phone→`tel:…`, sms→sms string, email→mailto string, text→text. `Content-Disposition: attachment; filename="qr4life-{hash}.png|svg"`.
...
Struktura homepage:
- Header: logo text „QR4Life", vpravo „Přihlásit se" (ghost) + „Registrovat" (accent button).
...
- [ ] `gh repo create bohous04/qr4life --public --source . --push` (remote origin). Expected: repo public na GitHubu.
...
- [ ] Vytvořit aplikaci z GitHub repa `bohous04/qr4life` (Dockerfile build), port 3000.
...
- [ ] Doména: pracovní Coolify URL; `qr4life.cz` až dodá DNS.
